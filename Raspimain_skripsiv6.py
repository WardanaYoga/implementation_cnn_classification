...
def run_gui():
    ...
    # ── FULLSCREEN ──────────────────────────────────────────
    # attributes -fullscreen  : true fullscreen (tanpa titlebar)
    # state zoomed             : maximize dengan titlebar (bisa minimize)
    # Di Raspi (Linux), pakai attributes -zoomed
    try:
        root.attributes("-zoomed", True)      # Linux / Raspberry Pi OS
    except tk.TclError:
        root.state("zoomed")                  # fallback Windows/Mac

    # Tekan F11 untuk toggle fullscreen tanpa titlebar
    # Tekan Escape untuk keluar fullscreen
    fullscreen_state = [False]
    def toggle_fullscreen(event=None):
        fullscreen_state[0] = not fullscreen_state[0]
        root.attributes("-fullscreen", fullscreen_state[0])
    ...
    # Ambil ukuran layar setelah window terbuka
    root.update_idletasks()
    SCREEN_W = root.winfo_screenwidth()
    SCREEN_H = root.winfo_screenheight()

    # Hitung ukuran panel secara proporsional terhadap layar
    CAM_PANEL_W  = int(SCREEN_W * 0.60)
    CAM_PANEL_H  = int(SCREEN_H * 0.65)
    INFO_PANEL_W = int(SCREEN_W * 0.32)
    INFO_PANEL_H = CAM_PANEL_H
    FONT_TITLE   = int(SCREEN_H * 0.028)
    FONT_LABEL   = int(SCREEN_H * 0.022)
    FONT_CLASS   = int(SCREEN_H * 0.055)
    FONT_SMALL   = int(SCREEN_H * 0.018)
    FONT_BTN     = int(SCREEN_H * 0.020)
    BTN_W        = int(SCREEN_W * 0.12)
    BTN_H        = int(SCREEN_H * 0.07)
    # ────────────────────────────────────────────────────────

    Label(root, text="SISTEM KLASIFIKASI SAMPAH",
          font=("Arial", FONT_TITLE, "bold"),
          fg="white", bg="#1e1e1e").pack(pady=int(SCREEN_H * 0.012))

    main_frame = Frame(root, bg="#1e1e1e")
    main_frame.pack(expand=True)

    # Kamera
    cam_frame = Frame(main_frame, bg="black",
                      width=CAM_PANEL_W, height=CAM_PANEL_H,
                      relief=tk.SUNKEN, borderwidth=2)
    cam_frame.grid(row=0, column=0, padx=int(SCREEN_W*0.01),
                   pady=int(SCREEN_H*0.01))
    ...
    info_frame = Frame(main_frame, bg="#2b2b2b",
                       width=INFO_PANEL_W, height=INFO_PANEL_H,
                       relief=tk.RAISED, borderwidth=2)
    info_frame.grid(row=0, column=1, padx=int(SCREEN_W*0.01),
                    pady=int(SCREEN_H*0.01))
    info_frame.pack_propagate(False)

    Label(info_frame, text="HASIL DETEKSI",
          font=("Arial", FONT_LABEL, "bold"),
          fg="white", bg="#2b2b2b").pack(pady=int(SCREEN_H*0.015))

    res_frame = Frame(info_frame, bg="#363636", relief=tk.GROOVE, borderwidth=2)
    res_frame.pack(pady=int(SCREEN_H*0.008), padx=12, fill=tk.BOTH)

    lbl_class = Label(res_frame, text="---",
                      font=("Arial", FONT_CLASS, "bold"),
                      fg="gray", bg="#363636", height=2)
    lbl_class.pack(pady=int(SCREEN_H*0.012))

    lbl_conf        = Label(info_frame, text="Confidence: -",
                            font=("Arial", FONT_LABEL), fg="white", bg="#2b2b2b")
    lbl_conf.pack(pady=int(SCREEN_H*0.005))
    lbl_entropy     = Label(info_frame, text="Entropy: -",
                            font=("Arial", FONT_SMALL), fg="lightgray", bg="#2b2b2b")
    lbl_entropy.pack(pady=int(SCREEN_H*0.003))
    lbl_consistency = Label(info_frame, text="Konsistensi: -",
                            font=("Arial", FONT_SMALL), fg="lightgray", bg="#2b2b2b")
    lbl_consistency.pack(pady=int(SCREEN_H*0.003))
    lbl_count       = Label(info_frame, text=f"Total: {detection_count}",
                            font=("Arial", FONT_SMALL), fg="lightblue", bg="#2b2b2b")
    lbl_count.pack(pady=int(SCREEN_H*0.005))
    lbl_status      = Label(info_frame,
                            text="⏳ Siapkan sampah, tekan MULAI",
                            font=("Arial", FONT_SMALL), fg="orange", bg="#2b2b2b",
                            wraplength=INFO_PANEL_W - 20, justify="center")
    lbl_status.pack(pady=int(SCREEN_H*0.008))

    # Tombol
    btn_frame = Frame(root, bg="#1e1e1e")
    btn_frame.pack(pady=int(SCREEN_H*0.010))
    ...
    btn_start = Button(btn_frame, text="▶ MULAI",
                       font=("Arial", FONT_BTN, "bold"), bg="green", fg="white",
                       width=BTN_W, height=BTN_H, command=on_start)
    btn_start.grid(row=0, column=0, padx=int(SCREEN_W*0.01))

    btn_reset = Button(btn_frame, text="🔄 ULANGI",
                       font=("Arial", FONT_BTN, "bold"), bg="#555", fg="white",
                       width=BTN_W, height=BTN_H, state=tk.DISABLED, command=on_reset)
    btn_reset.grid(row=0, column=1, padx=int(SCREEN_W*0.01))

    btn_exit = Button(btn_frame, text="✖ KELUAR",
                      font=("Arial", FONT_BTN, "bold"), bg="red", fg="white",
                      width=BTN_W, height=BTN_H, command=on_exit)
    btn_exit.grid(row=0, column=2, padx=int(SCREEN_W*0.01))
    ...
    def update():
        ...
        # Tampilkan ke Tkinter — resize sesuai ukuran panel dinamis
        frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        frame_res = cv2.resize(frame_rgb, (CAM_PANEL_W, CAM_PANEL_H))
